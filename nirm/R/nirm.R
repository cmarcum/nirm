...
### Please note that simplify reorders the edge list differently depending on use of  igraph or igraph0
### Consequently- to check that igraph and igraph0 produce same results, set R0 to Inf
g.PTN <- simplify(g.PTN)
...
for(flu.season in 1:total.flu.seasons){
  ...
  IDs.of.infected.clusters <- unique(g.sub.mem[initial.inf.samp])  
...
function (g, g.k, g.knn, seasonal.output, total.flu.seasons) 
{
...
function (g, x) 
{
    ...
    r <- if ("package:igraph0" %in% search()) {
        r <- subgraph(g, x)
    }
    else {
        r <- induced.subgraph(g, x)
    }
    return(r)
}
...
{
    offset <- ifelse("package:igraph0" %in% search(), -1, 0)
